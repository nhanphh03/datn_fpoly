...
public interface HoaDonChiTietRepository extends JpaRepository<HoaDonChiTiet, UUID> {
    ...
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet a join hoa_don b on a.id_hd=b.id_hd where month(b.tg_thanh_toan) = month(GETDATE()) \n" +
            "and year(b.tg_thanh_toan) = year(GETDATE()) and (loai_hd=1 and b.trang_thai=1) or (loai_hd=0 and b.trang_thai = 3)\n" +
            "            and day(b.tg_thanh_toan) = day(GETDATE())",nativeQuery = true)
    Optional<Integer> getTongSPBanTrongNgay();

    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet a join hoa_don b on a.id_hd=b.id_hd where month(b.tg_thanh_toan) = month(GETDATE())  and year(b.tg_thanh_toan) = year(GETDATE()) and" +
            "(loai_hd=1 and b.trang_thai=1) or (loai_hd=0 and b.trang_thai = 3)",nativeQuery = true)
    Optional<Integer> getTongSPBanTrongThang();
   ...
   Optional<Double> getTienGoc();
    ...
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 1 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang1();
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 2 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang2();
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 3 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang3();
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 4 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang4();
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 5 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang5();
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 6 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang6();
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 7 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang7();
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 8 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang8();
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 9 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang9();
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 10 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang10();
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 11 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang11();
    @Query(value = "select sum(so_luong) from hoa_don_chi_tiet join hoa_don h on h.id_hd=hoa_don_chi_tiet.id_hd where MONTH(tg_them) = 12 and (loai_hd=1 and h.trang_thai=1) or (loai_hd=0 and h.trang_thai = 3)",nativeQuery = true)
    Integer getThang12();
    ...
    @Query(value = "SELECT ctg.gia_ban,g.ten_giay ,hdct.don_gia_truoc_khi_giam,hdct.so_luong,ha.url1,hd.ten_nguoi_nhan,\n" +
            ...
            "WHERE (loai_hd=1 and hd.trang_thai=1) or (loai_hd=0 and hd.trang_thai = 3) AND MONTH(hd.tg_thanh_toan) = MONTH (getdate()) and Day(tg_thanh_toan) = Day(getdate())",nativeQuery = true)
    ...
    @Query(value = "SELECT ctg.gia_ban,g.ten_giay ,hdct.don_gia_truoc_khi_giam,hdct.so_luong,ha.url1,hd.ten_nguoi_nhan,\n" +
            ...
            "join hinh_anh ha on ha.id_hinh_anh=ctg.id_hinh_anh\n" +
            "join giay g on g.id_giay=ctg.id_giay\n" +
            "WHERE (loai_hd=1 and hd.trang_thai=1) or (loai_hd=0 and hd.trang_thai = 3) AND MONTH(hd.tg_thanh_toan) = MONTH (getdate()) and year(hd.tg_thanh_toan) = year (getdate())",nativeQuery = true)
    ...
    @Query(value = "select n.ma_nv,n.ho_ten_nv,n.email_nv,SUM(cthd.so_luong) FROM chi_tiet_giay ctg \n" +
            ...
            "\t\t\tand (loai_hd=1 and hd.trang_thai=1) or (loai_hd=0 and hd.trang_thai = 3)\n" +
